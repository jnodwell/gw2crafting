 ...
 * @since  1.6
 */
class Gw2crafterControllerRecipes extends JControllerAdmin
{

	protected $gw2API_v2_recipes = 'https://api.guildwars2.com/v2/recipes';
	 ...
	 * @since    1.6
	 */
	public function getModel($name = 'recipe', $prefix = 'Gw2crafterModel', $config = array())
	{
	...
	{
		$json = file_get_contents($this->gw2API_v2_recipes);
		$data = json_decode($json, true);
		$count = 0;
		foreach ($data as $key => $gw2_recipe_id) {
			$model = $this->getModel();
			if ($model->getRecipeByGw2Id($gw2_recipe_id) == 0) {
		...
		$this->setRedirect(JRoute::_('index.php?option=com_gw2crafting&view=recipes', false));

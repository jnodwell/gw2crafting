 ...
 * @since  1.6
 */
class PlgUserGw2profile extends JPlugin
{
	/**
	 * Date of birth.
	 *
	...
	public function onContentPrepareData($context, $data)
	{
		...
		if (is_object($data))
		{
			...
			if (!isset($data->gw2profile) && $userId > 0)
			{
				// Load the profile data from the database.
				$db = JFactory::getDbo();
				$db->setQuery(
					'SELECT profile_key, profile_value FROM #__user_profiles'
						. ' WHERE user_id = ' . (int) $userId . " AND profile_key LIKE 'gw2profile.%'"
						. ' ORDER BY ordering'
				);
				...
				$data->gw2profile = array();

				foreach ($results as $v)
				{
					$k = str_replace('gw2profile.', '', $v[0]);
					$data->gw2profile[$k] = json_decode($v[1], true);

					if ($data->gw2profile[$k] === null)
					{
						$data->gw2profile[$k] = $v[1];
					}
				}
			}
	...
	public function onContentPrepareForm($form, $data)
	{
		...
		{
			$form->setFieldAttribute('apikey', 'description', 'PLG_USER_GW2PROFILE_FILL_FIELD_DESC_SITE', 'gw2profile');
			$form->setFieldAttribute('website', 'description', 'PLG_USER_GW2PROFILE_FILL_FIELD_DESC_SITE', 'gw2profile');
			$form->setFieldAttribute('favoritebook', 'description', 'PLG_USER_GW2PROFILE_FILL_FIELD_DESC_SITE', 'gw2profile');
			$form->setFieldAttribute('aboutme', 'description', 'PLG_USER_GW2PROFILE_FILL_FIELD_DESC_SITE', 'gw2profile');
			$form->setFieldAttribute('dob', 'description', 'PLG_USER_GW2PROFILE_FILL_FIELD_DESC_SITE', 'gw2profile');
			$form->setFieldAttribute('tos', 'description', 'PLG_USER_GW2PROFILE_FIELD_TOS_DESC_SITE', 'gw2profile');
		}
	...
	public function onUserBeforeSave($user, $isnew, $data)
	{
		...
		if (!empty($data['gw2profile']['dob']))
		{
			try
			{
				$date = new JDate($data['gw2profile']['dob']);
				$this->date = $date->format('Y-m-d H:i:s');
			}
			catch (Exception $e)
			{
				// Throw an exception if date is not valid.
				throw new InvalidArgumentException(JText::_('PLG_USER_GW2PROFILE_ERROR_INVALID_DOB'));
			}
			if (JDate::getInstance('now') < $date)
			{
				// Throw an exception if dob is greather than now.
				throw new InvalidArgumentException(JText::_('PLG_USER_GW2PROFILE_ERROR_INVALID_DOB_FUTURE_DATE'));
			}
		}
		...
		if ($task === 'register' && $tosenabled && $tosarticle && $option === 'com_users' && !$data['gw2profile']['tos'])
		{
			throw new InvalidArgumentException(JText::_('PLG_USER_GW2PROFILE_FIELD_TOS_DESC_SITE'));
		}
	...
	public function onUserAfterSave($data, $isNew, $result, $error)
	{
		...
		if ($userId && $result && isset($data['gw2profile']) && count($data['gw2profile']))
		{
			try
			{
				$db = JFactory::getDbo();
				...
				// Sanitize the date
				$data['gw2profile']['dob'] = $this->date;

				$keys = array_keys($data['gw2profile']);
				...
				foreach ($data['gw2profile'] as $k => $v)
				{
			...
			try
			{
				...
				$db->setQuery(
					'DELETE FROM #__user_profiles WHERE user_id = ' . $userId
						. " AND profile_key LIKE 'gw2profile.%'"
				);
